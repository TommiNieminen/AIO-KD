...
python -m fairseq_cli.train ./data-bin/tc_Tatoeba-Challenge-v2023-09-26-tokenized.en-fi \
  ...
  --optimizer adam --lr 0.0005 -s en -t fi --label-smoothing 0.1 \
  ...
  --patience 10 --validate-interval-updates 5000 --student-mutual-learning no_weight \
  --warmup-updates 1000 --warmup-init-lr 1e-07 \
  --adam-betas '(0.9,0.98)' --save-dir ./ckpts/tc_Tatoeba-Challenge-v2023-09-26.en-fi.transformer_iwslt_de_en/stage2 \
  --no-epoch-checkpoints --fp16 --dropout 0.3 \
  --finetune-from-model ./ckpts/tc_Tatoeba-Challenge-v2023-09-26.en-fi.transformer_iwslt_de_en/stage1/checkpoint_best.pt --ddp-backend legacy_ddp \
  --save-interval-updates 20000 --keep-interval-updates 1 --eval-bleu --best-checkpoint-metric bleu --maximize-best-checkpoint-metric
